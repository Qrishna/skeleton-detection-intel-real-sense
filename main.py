...
def draw_skeleton(image):
	...
	for skel in data.skeletons:
		# print("Left Wrist: ", skel.left_wrist)
		coords = skel.right_wrist.real
		print("Right Wrist: %s,%s" % (coords[0], coords[1]))
		with open("html-p5/right-wrist.csv", "w") as file_writer:
			file_writer.write("%s,%s" % (coords[0], coords[1]))
		# class 'numpy.ndarray
		for el in skel[1:]:
